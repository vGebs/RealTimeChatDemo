...
class DocumentSnapshotPublisher<T: FirestoreProtocol>: Publisher {
    typealias Output = (T, DocChangeType)
    ...
    private final class DocumentSnapshotSubscription<S: Subscriber>: Subscription where S.Input == (T, DocChangeType), S.Failure == Error {
